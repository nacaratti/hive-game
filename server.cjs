...
const evaluateBoard = (board, botColor) => {
  ...
  // General Aggression: Reward being close to enemy queen
  if (enemyQueen) {
    ...
    surrounding.forEach(n => {
      const key = hexToString(n);
      const cell = board.find(([k]) => k === key);
      if (cell) {
        const stack = cell[1].stack;
...
const getBotMove = (currentPlayer, turnNumber, hand, board) => {
  ...
  if (canPlace) {
    ...
    for (const piece of piecesToTry) {
      for (const spot of spotsToCheck) {
        ...
        const newPiece = { id: 'sim', type: piece, color: currentPlayer };

        const existing = simBoard.find(([k]) => k === spot);
        if (existing) {
          existing[1].stack.push(newPiece);
        } else {
          simBoard.push([spot, { hex: stringToHex(spot), stack: [newPiece] }]);
        }
  ...
  // 2. Movements (only if queen placed)
  if (!queenNotPlaced && board.length > 0) {
    // Find my pieces
    const myPieces = [];
    ...
    // Use proper movement validation for each piece
    for (const hexStr of myPieces) {
      const validDestinations = getPieceMoves(board, hexStr);

      for (const dest of validDestinations) {
        const simBoard = cloneBoard(board);

        const originIdx = simBoard.findIndex(([k]) => k === hexStr);
        if (originIdx >= 0) {
          const movingPiece = simBoard[originIdx][1].stack.pop();
          if (simBoard[originIdx][1].stack.length === 0) {
            simBoard.splice(originIdx, 1);
          }

          const destIdx = simBoard.findIndex(([k]) => k === dest);
          if (destIdx >= 0) {
            simBoard[destIdx][1].stack.push(movingPiece);
          } else {
            simBoard.push([dest, { hex: stringToHex(dest), stack: [movingPiece] }]);
          }
...
io.on('connection', (socket) => {
  ...
  socket.on('game_action', (action) => {
    ...
    const validActionTypes = ['PLACE', 'MOVE', 'GAME_OVER'];
    ...
    if (action.type === 'GAME_OVER') {
      // Security: Validate GAME_OVER action structure
      if (!action.winner) {
        console.warn('Invalid GAME_OVER action structure from', socket.id);
        return;
      }

      // Security: Validate winner color
      if (!isValidColor(action.winner)) {
        console.warn('Invalid GAME_OVER winner color from', socket.id);
        return;
      }

      handleWin(action.winner);
    }
  });
